...
public:
  ...
  THnSparse * GetTHnSparse( TString name, Int1D dim, Int1D bin, Option_t*opt="");
  THnSparse * GetTHnSparse( TString name, Int1D dim, Int2D bin, Option_t*opt="");
...
THnSparse * BSTHnSparseHelper::GetTHnSparse( TString name, Int1D dim,  Int1D bin, Option_t*opt){
  Int2D newbin;
  for( auto b : bin ) newbin.push_back({b,b});
  return GetTHnSparse( name, dim, newbin, opt );
}
THnSparse * BSTHnSparseHelper::GetTHnSparse( TString name, Int1D dim, Int2D bin, Option_t*opt){
  //== Histogram Name and Title
  for (auto i : dim)  name+=Form("%sP%02d_",fH->GetName(),i);
  TString title = fH->GetTitle();
  for( UInt_t i=0;i<bin.size();i++ ){
    Int1D b = bin[i];
    Int1D nb = { 0, 0 };
    ...
    if( b[0]<-1 || b[1]<-1 || b[0] > maxnbin || b[1] > maxnbin || b[0] > b[1] ) ErrorExit(Form("Wrong bin : %i : %s : %d %d",i,ax->GetName(),b[0],b[1]));
    for (auto j : dim){ 
      if( int(i) == j || b.size()==0 || ( b[0]<=0 && b[1]<=0 ) ){
      }else{
        if( b[0] > 0 ) nb[0] = fCBinsB[i][b[0]];
        if( b[1] > 0 ) nb[1] = fCBinsB[i][b[1]+1]-1;
        //if( b0 > b1 ) b1 = b1;
	...
	Int_t newdim[dim.size()];
	int nn = 0;
	for (auto i : dim) { newdim[nn] = i ; nn ++ ;  };
  auto h = fH->Projection( dim.size(),newdim, opt );
  h->SetNameTitle(name,title);
  return h;
}
