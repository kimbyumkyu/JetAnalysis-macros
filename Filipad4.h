...
class Filipad4 {
    ...
    public:
        ...
        // ---- Costructor ------------
        Filipad4(int inID=1, float inRelSize=1.1, float inR = 0.4, int inXOffset = 100, int inYOffset=100, float inAspect=0.7, int ichop=5, int ichopPt=3){
        ...
        void Draw(){ 
            ...
            pp->SetTopMargin(MarginTop/(1-ratio)); pp->SetBottomMargin(0);//our pads will have no margin
            pp->SetLeftMargin(MarginLeft);pp->SetRightMargin(MarginRight);
            pp->Draw();

            pp = new TPad(Form("LPad%d1",ID),Form("LPad%d1",ID),0, 0, 1, (ratio-MarginBottom)/3. + MarginBottom ,0); //create pad
            pp->SetNumber(2);   //assign a number to it. Possible to access it via :  toppad->cd(ih); 
            pp->SetTopMargin(0); pp->SetBottomMargin(MarginBottom/( (ratio-MarginBottom)/3. + MarginBottom));//our pads will have no margin
            pp->SetLeftMargin(MarginLeft);pp->SetRightMargin(MarginRight);
            pp->Draw();
				
            pp = new TPad(Form("LPad%d2",ID),Form("LPad%d2",ID),0, (ratio-MarginBottom)/3 + MarginBottom , 1, (ratio-MarginBottom)/3*2.+ MarginBottom  ,0); //create pad
            pp->SetNumber(3);   //assign a number to it. Possible to access it via :  toppad->cd(ih); 
            pp->SetTopMargin(0); pp->SetBottomMargin(0);//our pads will have no margin
            pp->SetLeftMargin(MarginLeft);pp->SetRightMargin(MarginRight);
            pp->Draw();

						pp = new TPad(Form("LPad%d3",ID),Form("LPad%d3",ID),0, (ratio-MarginBottom)/3*2 + MarginBottom , 1,  ratio ,0); //create pad
            pp->SetNumber(4);   //assign a number to it. Possible to access it via :  toppad->cd(ih); 
            pp->SetTopMargin(0); pp->SetBottomMargin(0);//our pads will have no margin
            pp->SetLeftMargin(MarginLeft);pp->SetRightMargin(MarginRight);
            pp->Draw();
